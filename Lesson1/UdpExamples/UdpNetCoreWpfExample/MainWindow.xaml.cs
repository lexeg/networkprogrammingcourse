...
{
    private readonly StateObject _state = new();
    ...
    private void SendCompleted(IAsyncResult ar)
    {
        ...
        // Проверка специально для .NET Core приложений. В старом .Net Framework _sendResult не был null
        if (!ar.CompletedSynchronously) socket.EndSend(_sendResult);
    ...
    private void ReceiveCompleted(IAsyncResult ar)
    {
        try
        {
            if (ar.AsyncState is not StateObject stateObject)
            {
                return;
            }
